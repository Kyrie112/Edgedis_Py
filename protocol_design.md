...
### 数据分发细节
* 数据块id对应方式以及节点存储数据方式：由于发送数据的过程中存在两种两种数据块，一种是原始的数据块，可以通过纠删码解码完成；另一种时编码后的数据，可以直接从Sender处(或者后续的Leader补发机制)获取，故可以将数据块和编码块一起存储。例如设起始块号为a，总块数为k，需要n个块可以还原出原始文件，则id号对应关系应有：[a,a+k-1]存储编码块，[a+k,a+k+n-1]存储数据块
