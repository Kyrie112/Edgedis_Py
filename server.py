...
class Node:
    def __init__(self, server_host, server_port, server_id):
        ...
        self.max_id = 0  # receive max data id
        self.num = 2  # the num of servers
        self.term = 0  # need to be updated
        ...
        # here's some information for recover data
        self.file_ind = dict()  # store the erasure data block
        self.data_block_start_id = -1  # store the id of the first data block of current file
        self.original_data_block_cnt = -1  # store the original data block count(from )
        self.check_data_block_cnt = -1  # store the original data block count
        self.padlen = -1  # store the padlen from the cloud or sender
        self.new_file = False  # whether there is a new file
    ...
    def sign_in(self):  # need to be implemented
        ...
        for i in range(self.num):
            id = i + 1
            if id != self.id:
                client_thread = threading.Thread(target=self.connect_to_other,
                                                 args=(self.server_host_list[id], self.server_port_list[id], id),
                                                 name="connect_thread")
    ...
    '''
    # this function need to be changed after using erasure code in this process
    ...
    # here is how to handel the data message from sender after using erasure code
    # need to modify some messages in message.py
    def Data_Sender_Handle(self, mess):
        self.data_ind[mess.data_id] = mess.data  # also need to store the data block here(need to be updated later)
        self.file_ind[mess.data_id] = mess.data  # need to store here for using erasure code to decode data
        self.max_id = max(self.max_id, mess.data_id)
        file_status = not self.new_file  # if there is no new file coming, it means already received the file
        # added after using erasure code
        if mess.data_block_start_id > self.data_block_start_id:  # new file coming
            self.data_block_start_id = mess.data_block_start_id
            self.padlen = mess.padlen
            self.new_file = True
        logger.debug(
            f"Received data block from {mess.id}... [data_id: {mess.data_id}, data_len: {len(mess.data)}, ip: {mess.from_host}, port: {mess.from_port}]")
        file_status = self.Decode_Code_Blocks() # decode and check whether it has decoded the original data
        # need to response
        mess_response = message.Message_Data_Sender_Response(self.id, mess.data_id, True, file_status, self.server_host,
                                                             self.server_port)
        self.Send(mess.id, self.receive_clients[mess.id], mess_response)

    def Data_Sender_Response_Handle(self, mess):
        logger.debug(
            f"Received response from {mess.id}... [data_id: {mess.data_id}, ip: {mess.from_host}, port: {mess.from_port}]")
        n = self.num
        ...
        if len(self.store_map) > int((n + 1) / 2):  # need to tell the cloud that the data block has been transmitted
            ...
            mess_response = message.Message_Data_Client_Response(self.id, mess.data_id, True, self.server_host,
                                                                 self.server_port)
            self.Send(0, self.receive_clients[0], mess_response)

    def Data_Client_Handle(self, mess):
        logger.debug(
            f"Received data block from cloud... [data_id: {mess.data_id}, data_len: {len(mess.data)}, ip: {mess.from_host}, port: {mess.from_port}]")
        ...
        # added after using erasure code
        self.original_data_block_cnt = mess.original_data_block_cnt
        self.check_data_block_cnt = mess.check_data_block_cnt
        self.padlen = mess.padlen

        mess_send = message.Message_Data_Sender(self.id, mess.data, self.padlen, mess.data_id, self.server_host, self.server_port)
    ...
    """
    # need to update here so that it can supply missing data with erasure code
    ...
    """
    # need to update here for using erasure code
    def Data_Supplement_Handle(self, mess):
        for ind in range(len(mess.request_list)):
            self.data_ind[mess.reuest_list[ind]] = mess.request_data[ind]   # store it as usual
            if self.new_file :  # if the server still don't have the original data
                self.file_ind[mess.request_list[ind]] = mess.request_data[ind]  # need to store it as well
        file_status = self.Decode_Code_Blocks() # after receive the data blocks it missed, check whether it can get the original data
        if file_status :
            print(" get the original data through candidate recovery")
    ...
    def Decode_Code_Blocks(self):
        file_status = False
        if self.file_ind.__len__() >= self.original_data_block_cnt and self.new_file:  # enough to recover the
            # original data and here is a new file
            current_data_blocks = []
            current_data_blocks_ind = []
            for ind in range(0, self.original_data_block_cnt + self.check_data_block_cnt):
                if self.data_block_start_id + ind in self.file_ind.keys():
                    current_data_blocks.append(self.file_ind[self.data_block_start_id + ind])
                    current_data_blocks_ind.append(ind)
            original_data = erasure_code.decoded_data(current_data_blocks, current_data_blocks_ind,
                                                      self.original_data_block_cnt,
                                                      self.check_data_block_cnt, self.padlen)
            print("get original data: ", original_data)
            self.new_file = False
            file_status = True  # already received the file
            self.file_ind.clear()
        return file_status
...
if __name__ == "__main__":
    ...
    local_ip, local_port, local_id = config["server_host_private"][args.server_id - 1], config["server_port"][
        args.server_id - 1], args.server_id
